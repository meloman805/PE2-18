...
; 0000 0046 ACSR=1<<ACD;        //В исходном состоянии AC выключен, включается
	LDI  R30,LOW(128)
	OUT  0x8,R30
; 0000 0047 SFIOR=0x00;         //и подключается к схеме захвата при запуске измерения
	LDI  R30,LOW(0)
	OUT  0x30,R30
; 0000 0048 
...
; .FSTART _ext_int1_isr
	ST   -Y,R30
; 0000 0055 TCNT0=0x00;        //Обнулить TC1 для подготовки к измерению
	LDI  R30,LOW(0)
	OUT  0x32,R30
; 0000 0056 
; 0000 0057 TCCR0=0xC5;        //запустить СТ1 на счет с коэффициентом деления 8 (f=1МГц)
	LDI  R30,LOW(197)
	OUT  0x33,R30
; 0000 0058                     //системная тактовая частота 8 МГц
; 0000 0059 ACSR=1<<ACIC;       //Включить аналоговый компаратор и подкл. его к схеме захвата
	LDI  R30,LOW(4)
	OUT  0x8,R30
; 0000 005A }
	LD   R30,Y+
...
; 0000 005E {   //разрядить емкость
_timer1_capt_isr:
; .FSTART _timer1_capt_isr
	ST   -Y,R30
; 0000 005F PORTA = TCNT0;          //вывести результат измерения на PORTA
	IN   R30,0x32
	OUT  0x1B,R30
; 0000 0060 ACSR=1<<ACD;            //Отключить аналоговый компаратор во избежании
	LDI  R30,LOW(128)
	OUT  0x8,R30
