...
{
TCNT0=0x00;        //Обнулить TC1 для подготовки к измерению

TCCR0=0xC5;        //запустить СТ1 на счет с коэффициентом деления 8 (f=1МГц)
...
{   //разрядить емкость
PORTA = TCNT0;          //вывести результат измерения на PORTA 
