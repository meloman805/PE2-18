...
while ((ADCSRA & 0x10)==0);             /*ожидаем окончания преобразования
                                        (пока не установился флаг прерывания АЦП)*/
ADCSRA|=0x10;                           //сбрасываем флаг готовности и считываем данные
...
read_adc(0x0B);         //измерение дифференциального сигнала
PORTB=ADCL;             //ADC1-ADC0 с коэфф усиления 200x
PORTC=ADCH;             //вывод результата в порты C, B
}
...
PORTC=0x00;             //4 мл. бита порта С - на вывод данных
DDRC=0x0F;              //для вывода старшего байта преобразованных АЦП данных
...
// INT1: Off
// INT2: Off
GICR|=0x40;             //Разрешение прерывания INT0
MCUCR=0x02;             //Запуск прерывания - по заднему фронту INT0
MCUCSR=0x00;
GIFR=0x40;
