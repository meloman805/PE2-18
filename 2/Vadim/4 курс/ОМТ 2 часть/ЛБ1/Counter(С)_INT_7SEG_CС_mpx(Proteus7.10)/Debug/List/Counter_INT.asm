...
; 0000 0024 PORTB=0;
	...
	OUT  0x18,R30
; 0000 0025 
; 0000 0026 // Port C initialization На вывод для зажигания сегментов графики знака
...
; 0000 003B       unsigned char i;                      //индекс элемента массива шестнадцатеричных цифр
; 0000 003C       for (i=0; i<4; i++)                   //отображение по очереди всех цифр числа
	SBIW R28,1
;	i -> Y+0
	LDI  R30,LOW(0)
	ST   Y,R30
_0xA:
	...
	BRSH _0xB
; 0000 003D             {
; 0000 003E             PORTB=0b1000>>i;                //Активизируем очередной общий катод
	LD   R30,Y
	LDI  R26,LOW(8)
	CALL __LSRB12
	OUT  0x18,R30
; 0000 003F             PORTC=GRAPH_HEX[DIGITS[i]];     //Код 7-сег. дисплея для очередного знакоместа (очередная цифра)
	LD   R30,Y
	LDI  R31,0
	SUBI R30,LOW(-_DIGITS)
	SBCI R31,HIGH(-_DIGITS)
	LD   R30,Z
	LDI  R31,0
	SUBI R30,LOW(-_GRAPH_HEX*2)
	SBCI R31,HIGH(-_GRAPH_HEX*2)
	LPM  R0,Z
	OUT  0x15,R0
; 0000 0040             delay_ms(3);                    //Задержка для подсветки очередного знакоместа
	LDI  R26,LOW(3)
	LDI  R27,0
	CALL _delay_ms
; 0000 0041             PORTB=0x00;                     //Гашение текущего знакоместа (нужно только для инд. PROTEUS)
	LDI  R30,LOW(0)
	OUT  0x18,R30
; 0000 0042             //PORTC=0;                        //Для реальных в гашении нет необходимости (если только
; 0000 0043             delay_us(5);                    //не стоит задача регулировки яркости свечения индикатора)
	__DELAY_USB 20
; 0000 0044             }
	LD   R30,Y
	SUBI R30,-LOW(1)
